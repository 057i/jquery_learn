(function () {
    ...
    jQuery.prototype.myDequeue = function (type) {
        var self = this;
        var queueName = arguments[0] || 'fx';
        var queueArr = this.myQueue(queueName);//递归调用自身,这个括号里的queueName()是执行dequeue(数组名)时候传进去的队列数组名
        console.log(queueArr)
        var currFunc = queueArr.shift();//数组方法shift，执行几次dequeue出队操作就有几个currFunc!!!
    ...
    jQuery.prototype.myDelay = function (duration) {  //括号里填的是过度时间
        var queueArr = this[0]['fx'];  //'fx'在队列事件中表示的是animate的接口
        console.log(queueArr)
